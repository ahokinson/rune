...
[rune](README.md) / TriggerVolume

# Class: TriggerVolume

Defined in: physics/triggers.ts:18
...
```ts
new TriggerVolume(options?): TriggerVolume;
```
...
```ts
triggerMask: number = CollisionLayer.all;
```

Defined in: physics/triggers.ts:20
...
```ts
evaluateOverlap(other): void;
```

Defined in: physics/triggers.ts:37
...
does not match [triggerMask](#triggermask).
...
```ts
onOverlapEnter(_other): void;
```

Defined in: physics/triggers.ts:24
...
```ts
onOverlapExit(_other): void;
```

Defined in: physics/triggers.ts:28
...
```ts
onOverlapStay(_other): void;
```

Defined in: physics/triggers.ts:26
