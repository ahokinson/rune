...
[rune](README.md) / SpriteEntity

# Class: SpriteEntity

Defined in: scene/spriteEntity.ts:27

The simplest renderable: an [Entity2D](Class.Entity2D.md) that draws a single [Sprite](Class.Sprite.md)
at its position.
...
```ts
new SpriteEntity(options): SpriteEntity;
```

Defined in: scene/spriteEntity.ts:35
...
Construction options. `size` defaults to the sprite's
  dimensions.
...
Defined in: scene/spriteEntity.ts:29
...
Defined in: scene/spriteEntity.ts:49
