...
[rune](README.md) / WorldView3D

# Class: WorldView3D

Defined in: scene/worldView3d.ts:34

The bridge that makes "everything is an entity" hold for 3D: a normal
[Entity2D](Class.Entity2D.md) that owns a [Scene3D](Class.Scene3D.md), ticks it each fixed update, and
draws it at a viewport.

Drop it into a 2D [Scene](Function.Scene.md) and 2D HUD/effect entities layer around it by
...
```ts
new WorldView3D(options): WorldView3D;
```

Defined in: scene/worldView3d.ts:43
...
Defined in: scene/worldView3d.ts:38
...
Defined in: scene/worldView3d.ts:36
...
```ts
draw(canvas, _camera): void;
```

Defined in: scene/worldView3d.ts:77
...
Unused; the 3D scene's own camera is used.
...
Defined in: scene/worldView3d.ts:67
...
```ts
protected viewportFor(canvas): Viewport3D;
```

Defined in: scene/worldView3d.ts:56

The viewport to draw the world at this frame. Defaults to the fixed
[viewport](#viewport), or a centered viewport filling the canvas if none was given.
